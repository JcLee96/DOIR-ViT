...
class Classify4LitModule(LightningModule):
    def __init__(
        ...
        self.train_time = []
        self.eval_time = []
        self.train_gpu_memory_bytes = 0
        self.eval_gpu_memory_bytes = 0

        self.train_start_time, self.train_end_time = 0, 0
        self.eval_start_time, self.eval_end_time = 0, 0
    ...
    def training_step(self, batch, batch_idx):
        ...
        train_elapsed_time = self.train_end_time - self.train_start_time
        self.train_gpu_memory_bytes = torch.cuda.memory_allocated(device='cuda')
        self.train_time.append(train_elapsed_time * 1e6)
    ...
    def training_epoch_end(self, outputs: List[Any]):
        ...
        #
        # timm_model = nn.Sequential(
        #     *list(self.model.children())[:-1],
        #     self.discriminator_layer1
        # )
        #
        # toy_input = torch.randn(1, 3, 512, 512)
        # flops, params = profile(timm_model.to('cpu'), inputs=(toy_input,))  # compute FLOPs and parameters
        #
        # # print(f"TRAIN {self.model_name} Model size: {self.train_gpu_memory_bytes / (384 * 384):.2f} MB")
        # # print(f"EVAL {self.model_name} Model size: {self.eval_gpu_memory_bytes / (384 * 384):.2f} MB")
        # print('FLOPs:', flops)
        # print('train_time SUM', np.array(self.train_time).sum())
        # # print('eval_time SUM', np.array(self.eval_time).sum())
        # print('train_time MEAN', np.array(self.train_time).mean())
        # # print('eval_time MEAN', np.array(self.eval_time).mean())
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # eval_elapsed_time = self.eval_end_time - self.eval_start_time
        # self.eval_gpu_memory_bytes = torch.cuda.memory_allocated(device='cuda')
        # self.eval_time.append(eval_elapsed_time * 1e6)
    ...
    def validation_epoch_end(self, outputs):
        ...
        toy_input = torch.randn(1, 3, 384, 384)
        flops, params = profile(timm_model.to('cpu'), inputs=(toy_input,))  # compute FLOPs and parameters
